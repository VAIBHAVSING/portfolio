...
function loadContributionsFromCache(): CachedContributionsData | null {
  try {
    ...
    if (!fs.existsSync(cacheFilePath)) {
      console.warn(
        "Cache file not found. Run: npm run update-contributions-cache",
      );
      return null;
    }
    ...
    console.log(
      `Cache age: ${Math.round(cacheAge / (1000 * 60))} minutes (fresh: ${isCacheFresh})`,
    );

    if (!isCacheFresh) {
      console.warn(
        "Cache is stale. Consider running: npm run update-contributions-cache",
      );
    }
...
export async function GET(request: Request) {
  ...
  try {
      ...
      return NextResponse.json(
        {
          ...
          message:
            "Contributions cache not found. Please run the cache generation script.",
    ...
    console.log(
      `API: Served ${cacheData.total} contributions from cache in ${Date.now() - startTime}ms`,
    );
    console.log(
      `Breakdown: ${cacheData.breakdown.merged} merged, ${cacheData.breakdown.open} open, ${cacheData.breakdown.closed} closed`,
    );
