...
export const LandingHero: React.FC<LandingHeroProps> = ({ words }) => {
  return (
    <section id="about" className="relative w-full min-h-screen flex flex-col items-center justify-center px-6 py-12 md:py-20 overflow-hidden">
      <div className="max-w-[80%] mx-auto text-center relative z-10">
        <div className="mb-4">
          <div className="w-24 h-24 mx-auto rounded-full overflow-hidden border-4 border-primary/20 shadow-2xl">
            <Image src={PERSONAL_INFO.avatar} alt={PERSONAL_INFO.name} width={96} height={96} className="w-full h-full object-cover" />
          </div>
        </div>
        <div className="mb-2">
          <h1 className="text-3xl md:text-5xl font-light text-foreground mb-2 font-sans tracking-tight">
            Hi, I&apos;m <span className="bg-gradient-to-r from-primary via-primary/80 to-primary/60 bg-clip-text text-transparent font-semibold">{PERSONAL_INFO.name}</span>
          </h1>
        </div>
      ...
      <div className="absolute bottom-20 left-20 w-2 h-2 bg-primary/30 rounded-full animate-pulse delay-1000" />
      <div className="absolute bottom-32 left-32 w-3 h-3 bg-primary/20 rounded-full animate-pulse delay-1300" />
      <div className="absolute bottom-40 left-16 w-1 h-4 bg-primary/40 animate-pulse delay-1500" />
    </section>
  );
};
