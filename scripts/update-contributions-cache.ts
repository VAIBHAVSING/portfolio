...
async function getOrganizationAvatar(owner: string): Promise<string | undefined> {
  try {
    ...
    for (const prItem of batch) {
      try {
        ...
        let record: ContributionRecord = {
          ...
          title: prItem.title,
          url: prItem.html_url,
          state: prItem.state === 'open' ? 'open' : (isMerged ? 'merged' : 'closed'),
        ...
        if (prNumber > 0 && owner && repo) {
          try {
            ...
            console.log(`✅ ${repoFullName}#${prNumber} is ${record.state.toUpperCase()}`);
            
          } catch (detailError) {
            // For enterprise repos with token restrictions, use search API data
            console.warn(`⚠️  Failed to get details for ${repoFullName}#${prNumber}, using search data:`, 
                        detailError instanceof Error ? detailError.message : 'Unknown error');
            
            // Use what we can from the search API
            if (isMerged) {
              record.state = 'merged';
              console.log(`✅ ${repoFullName}#${prNumber} is MERGED (from search API)`);
            } else {
              console.log(`📝 ${repoFullName}#${prNumber} is ${record.state.toUpperCase()} (from search API)`);
            }
            ...
            try {
              ...
              record.ownerAvatar = repoResponse.data.owner.avatar_url || undefined;
            } catch (repoError) {
...
async function main() {
  ...
  try {
    ...
    filteredResults.sort((a, b) => new Date(b.created).getTime() - new Date(a.created).getTime());
